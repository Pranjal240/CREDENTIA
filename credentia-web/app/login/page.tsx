...
const roles = [
  ...
  { id: 'admin', emoji: '🔐', label: 'Admin', desc: 'Platform management', color: 'from-violet-500 to-violet-600' },
]
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={{ background: 'rgb(var(--bg-base))' }}>
      ...
      <div className="absolute w-[400px] h-[400px] rounded-full bg-teal-500/8 blur-[100px] -bottom-32 -right-32" />
      <div className="absolute inset-0 opacity-[0.02]" style={{ backgroundImage: 'radial-gradient(circle, rgb(var(--accent)) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-syne text-2xl font-extrabold" style={{ color: 'rgb(var(--text-primary))' }}>Welcome Back</h1>
                ...
                {roles.map((r) => (
                  <button
                    ...
                    style={{
                      ...
                      borderColor: 'rgba(var(--border-default), 0.6)',
                    }}
                    onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(var(--accent), 0.4)'; e.currentTarget.style.boxShadow = '0 0 20px rgba(var(--accent-glow),0.1)' }}
                    onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(var(--border-default), 0.6)'; e.currentTarget.style.boxShadow = 'none' }}
                  >
                    <span className="text-2xl block mb-2">{r.emoji}</span>
                    <span className="font-syne font-bold text-sm block" style={{ color: 'rgb(var(--text-primary))' }}>{r.label}</span>
