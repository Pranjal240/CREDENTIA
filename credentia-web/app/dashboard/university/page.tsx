...
export default async function UniversityDashboard() {
  ...
  const students = (rawStudents || []).map((s: any) => {
    ...
    return {
      ...
      degree_verified: s.verifications?.some((v: any) => v.type === 'degree' && ['verified', 'ai_approved', 'admin_verified'].includes(v.status)) ?? false,
      police_verified: s.verifications?.some((v: any) => v.type === 'police' && ['verified', 'ai_approved', 'admin_verified'].includes(v.status)) ?? false,
      aadhaar_verified: s.verifications?.some((v: any) => v.type === 'aadhaar' && ['verified', 'ai_approved', 'admin_verified'].includes(v.status)) ?? false,
