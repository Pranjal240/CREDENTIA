          ...
          {showFilters && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
              <div className="px-4 pb-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                {[
                  { label: 'Degree Status', value: filterDegree, onChange: setFilterDegree, options: [['all', 'All'], ['verified', 'Verified'], ['pending', 'Pending']] },
                  { label: 'Police Status', value: filterPolice, onChange: setFilterPolice, options: [['all', 'All'], ['verified', 'Verified'], ['pending', 'Pending']] },
                ].map((f, i) => (
                  <div key={i} className="pt-3">
                    <label className="text-[9px] text-white/25 uppercase tracking-wider mb-1 block">{f.label}</label>
                    <select value={f.value} onChange={e => { f.onChange(e.target.value); setPage(1) }} className="w-full h-9 px-2 rounded-lg text-xs bg-white/5 border border-white/10 text-white/70 focus:outline-none">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden shadow-2xl">
        ...
        {paginated.length === 0 ? (
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-white/40 max-w-md mx-auto">{students.length === 0 ? 'Students can link their profiles to your university from their Dashboard → Settings → "Link to University". Once linked, they appear here.' : 'Try adjusting your filters.'}</p>
          </div>
        ) : (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-2xl rounded-2xl border border-indigo-500/30 bg-[#0e0e14] shadow-2xl shadow-indigo-500/10 overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="relative p-6 border-b border-white/10 bg-white/[0.02]">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-xl bg-indigo-500/20 text-indigo-400">✨</div>
                    <div>
              ...
              <div className="p-8">
                {loadingInsights ? (
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                    {aiInsights?.split('\n').map((paragraph, i) => (
                      <p key={i} className={paragraph.startsWith('-') ? 'ml-4 flex items-start gap-2 text-sm text-white/70' : 'text-sm text-white/70'}>
