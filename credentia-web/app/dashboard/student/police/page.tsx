...
export default function PolicePage() {
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      setResult(resData.analysis || resData)
      setFileUrl(resData.fileUrl || link)
      setSaveStatus(resData.status || (resData.analysis?.is_police_certificate ? 'ai_approved' : 'rejected'))
      setAnalyzing(false)
    } catch (err: any) { setError(err.message); setAnalyzing(false) }
  }

  const reset = () => { setFile(null); setLink(''); setResult(null); setError(''); setSaved(false); setFileUrl(''); setSaveStatus('') }
      ...
      {!result ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 mt-8 pt-6 border-t border-white/5">
            {!saved ? (
              <button onClick={handleSave} disabled={saving} className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl font-bold transition-all shadow-lg shadow-violet-500/20" style={{ background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', color: 'white' }}>
                {saving ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />}
                {saving ? 'Saving...' : 'SAVE TO PROFILE'}
              </button>
            ) : (
              <div className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl font-bold bg-violet-500/10 text-violet-400 border border-violet-500/20">
