...
export default function PolicePage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const analyzeRes = await fetch('/api/verify-police', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: data.url, filename: data.filename }) })
      if (!analyzeRes.ok) throw new Error('Verification failed')
      setResult(await analyzeRes.json()); setAnalyzing(false)
  ...
  const handleLink = async () => {
    ...
    try {
      const res = await fetch('/api/verify-police', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: link, type: 'link' }) })
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="font-syne text-2xl font-bold" style={{ color: 'rgb(var(--text-primary))' }}>Police Verification</h1>
              ...
              <div {...getRootProps()} className="border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer transition-all" style={{ borderColor: file ? 'rgb(var(--success))' : 'rgba(var(--border-default), 0.6)' }}>
                ...
                {file ? (
                  <div className="flex flex-col items-center gap-2">
                    <FileText size={36} style={{ color: 'rgb(var(--success))' }} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="rounded-2xl p-6 border" style={{ background: 'rgb(var(--bg-card))', borderColor: 'rgba(var(--border-default), 0.5)' }}>
            <div className="flex items-center gap-3 mb-5">
              {result.is_police_certificate ? <CheckCircle2 size={24} style={{ color: 'rgb(var(--success))' }} /> : <AlertCircle size={24} style={{ color: 'rgb(var(--danger))' }} />}
              <div>
                <p className="font-syne font-bold" style={{ color: 'rgb(var(--text-primary))' }}>{result.is_police_certificate ? 'Valid Police Certificate' : 'Document Not Recognized'}</p>
